...
public class dialogObjScr : MonoBehaviour
{
    ...
    {
        CurrentMessageNum = 0;
        CurrentDialog = DialogManager.Dialogs.Find(x => x.Type == type); //ищем все диалоги нужно нам типа

        transform.DOLocalMoveY(-228.5f, 1);                                  //меняем положение блока по оси Y с анимацией
                                                                    //нужно будет сделать какую-нибудь анимацию
        ShowMessage();
    }
    ...
    void EndDialog()
    {
        ...
        transform.DOLocalMoveY(-328.5f, 1);         //смещаем блок диалога обратно
    }
}
